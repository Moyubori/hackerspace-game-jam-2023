        ...
        UseStateController<GoblinController>
{
  Goblin(Vector2 position)
    ...
    position: position,
    size: Vector2.all(DungeonProvider.tileSize * 0.8),
    speed: DungeonProvider.tileSize * 1.6,
    life: 100,
  ) {
    setupCollision(
      CollisionConfig(
        collisions: [
          CollisionArea.rectangle(
            size: Vector2(
              DungeonProvider.tileSize * 0.4,
              DungeonProvider.tileSize * 0.4,
            ),
            align: Vector2(
              DungeonProvider.tileSize * 0.2,
              DungeonProvider.tileSize * 0.2,
            ),
          ),
        ],
  ...
  void die() {
    ...
    gameRef.add(
      AnimatedObjectOnce(
        ...
        position: position,
        size: Vector2.all(DungeonProvider.tileSize),
      ),
    );
    removeFromParent();
  }
  ...
  void execAttackRange(double damage) {
    ...
    simpleAttackRange(
      ...
      damage: damage,
      speed: DungeonProvider.tileSize * 3,
  ...
  void execAttack(double damage) {
    ...
    simpleAttackMelee(
      ...
      damage: damage / 2,
      interval: 400,
      sizePush: DungeonProvider.tileSize / 2,
